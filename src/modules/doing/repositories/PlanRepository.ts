...
export class PlanRepository {
  ...
  private async create(plan: Plan) {
    ...
    const params = [plan.id, plan.churchId, plan.ministryId, plan.planTypeId, plan.name, plan.serviceDate?.toISOString().split("T")[0] || new Date().toISOString().split("T")[0], plan.notes, plan.serviceOrder, plan.contentType, plan.contentId];
  ...
  private async update(plan: Plan) {
    ...
    const params = [plan.ministryId, plan.planTypeId, plan.name, plan.serviceDate?.toISOString().split("T")[0] || new Date().toISOString().split("T")[0], plan.notes, plan.serviceOrder, plan.contentType, plan.contentId, plan.id, plan.churchId];
    await DB.query(sql, params);
    return plan;
  }
