...
**Listen gate.** `POST /commons/admin/songs/:id/listen { keys: string[] }` (reviewer, see `helpers/ReviewAccess.ts`) records the keys heard; covering every `publishedKeys` entry on a package that serves a score, chords and slides makes it `sunday-ready`, and `{ keys: [] }` clears back to the computed tier.
