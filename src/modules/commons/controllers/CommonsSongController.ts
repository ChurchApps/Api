...
export class CommonsSongController extends CommonsBaseController {
  ...
  private async detail(song: SongView): Promise<SongDetail> {
    ...
    const readText = async (name: string) => (await ContentLibraryHelper.readKey(ContentLibraryHelper.fileKey(asset, files, name)))?.buffer.toString("utf8") ?? null;
    return await SongPackageHelper.detail(song, ContentLibraryHelper.fileUrls(asset, files, song.portraitKey), { readText });
  }
