...
export const songPublishHook: PublishHook = {
  async onPublish(ctx) {
    ...
    if (song.chordPro) song.chordPro = SongPackageHelper.letterChords(SongPackageHelper.inlineChordLines(SongPackageHelper.dropTitleLine(SongPackageHelper.tidyPaste(String(song.chordPro).replace(/\r\n/g, "\n")), asset.name)), song.songKey); // library files are LF
