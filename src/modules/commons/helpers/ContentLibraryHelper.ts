...
// fileRole() lives in @churchapps/helpers, so the extra names are mapped here rather than there.
const PACKAGE_ROLES: Record<string, string> = { "score.musicxml": "score", "slides.json": "slides", "chart.chordpro": "chart", "chart.pdf": "chartPdf", "attribution.txt": "attribution", "cover-thumb.webp": "thumb", "duration.json": "duration" };
// when two files share a role the freshest wins: lyrics.chordpro is rewritten on every publish, an uploaded art-thumb outranks the generated cover thumb
const PREFERRED = new Set(["lyrics.chordpro", "art-thumb.webp"]);
...
export class ContentLibraryHelper {
  ...
  static role(name: string): string {
    return PACKAGE_ROLES[name] ?? fileRole(name);
  }
  ...
  static fileUrls(asset: { assetType?: string; id?: string }, files: AssetFile[], portraitKey?: string): Record<string, string> {
    ...
    for (const f of files) {
      ...
      if (!(role in out) || PREFERRED.has(f.name)) out[role] = this.publicUrl(this.liveKey(asset, f.name));
    }
